...
function App() {
  ...
  const { data: siteData, error: siteError, isLoading: siteLoading } = useContentSite(instanceIdResolved, exToken);
...
function Layout({ site, instanceId }: { site: Site; instanceId?: string | null | undefined }) {
  return (
    <>
      <Header site={site} instanceId={instanceId} />
      <Outlet />
      <SiteFooter />
    </>
  );
