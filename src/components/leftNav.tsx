...
const LeftNav: React.FC<LeftNavProps> = ({ site, usingDefaultPage, pageIdentifier = "" }) => {
  ...
  useEffect(() => {
    ...
    const selected = site?.content?.globals?.navigation?.find((item) => item.links?.find((child) => pageIdentifier.endsWith(child.slug)));
  ...
  const collaspeMenu = import.meta.env.VITE_COLLASPE_LEFT_MENU === "true";
              ...
              {site?.content?.globals?.navigation.map((item) => {
                return (
                  <li key={item.group}>
                    ...
                    {(selected === item.group || !collaspeMenu) && (
